    ...
    <>
      ...
      <main className={`min-h-screen p-0 md:p-6 transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-gray-200' : 'bg-gradient-to-br from-green-50 via-green-100 to-green-200 text-emerald-900'}`}>
        <header className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <h1 className={`text-3xl md:text-4xl font-extrabold ${darkMode ? 'text-white' : ''}`}>Dashboard Warga</h1>
        </header>
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className={`rounded-xl shadow-lg p-6 flex items-center gap-4 border-l-8 animate-fadein
            ...
            <div>
              <div className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-cyan-900'}`}>2</div>
              <div className={`text-sm font-semibold ${darkMode ? 'text-cyan-200' : 'text-cyan-700'}`} style={darkMode ? {opacity: 0.85} : {}}>Peminjaman Aktif</div>
            </div>
          </div>
          <div className={`rounded-xl shadow-lg p-6 flex items-center gap-4 border-l-8 animate-fadein
            ${darkMode ? 'bg-gray-800/90 border-cyan-700' : 'bg-white border-cyan-400'}`}>
            <FaHistory className={`text-3xl ${darkMode ? 'text-cyan-300' : 'text-cyan-600'}`} />
            <div>
              <div className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-cyan-900'}`}>5</div>
              <div className={`text-sm font-semibold ${darkMode ? 'text-cyan-200' : 'text-cyan-700'}`} style={darkMode ? {opacity: 0.85} : {}}>Riwayat Peminjaman</div>
            </div>
          </div>
        </section>
        <section className={`rounded-2xl shadow-xl p-6 mb-10 animate-fadein-slow ${darkMode ? 'bg-gray-800/90' : 'bg-white'}`}>
          <div className="flex justify-between items-center mb-4">
            <h2 className={`text-xl font-bold ${darkMode ? 'text-white' : ''}`}>Status Peminjaman</h2>
            <Link href="#" className={`flex items-center gap-2 hover:underline font-bold ${darkMode ? 'text-cyan-200' : 'text-cyan-700'}`}><FaPlusCircle /> Ajukan Peminjaman</Link>
          </div>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                <tr className={`${darkMode ? 'border-b border-cyan-800 hover:bg-cyan-950' : 'border-b hover:bg-cyan-50'}`}>
                  ...
                  <td className="py-2 px-4"><span className={`${darkMode ? 'bg-cyan-800 text-cyan-100' : 'bg-cyan-200 text-cyan-800'} px-2 py-1 rounded font-bold`}>Disetujui</span></td>
                </tr>
                <tr className={`${darkMode ? 'border-b border-cyan-800 hover:bg-cyan-950' : 'border-b hover:bg-cyan-50'}`}>
                  ...
                  <td className="py-2 px-4"><span className={`${darkMode ? 'bg-yellow-700 text-yellow-200' : 'bg-yellow-200 text-yellow-800'} px-2 py-1 rounded font-bold`}>Menunggu</span></td>
                </tr>
                <tr className={`${darkMode ? 'hover:bg-cyan-950' : 'hover:bg-cyan-50'}`}>
                  ...
                  <td className="py-2 px-4"><span className={`${darkMode ? 'bg-red-800 text-red-200' : 'bg-red-200 text-red-800'} px-2 py-1 rounded font-bold`}>Ditolak</span></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <section className="flex flex-wrap gap-4">
          <Link href="#" className={`px-6 py-3 rounded-xl shadow font-bold transition animate-bouncein
            ${darkMode ? 'bg-cyan-700 text-white hover:bg-cyan-800' : 'bg-cyan-600 text-white hover:bg-cyan-700'}`}
          >Ajukan Peminjaman</Link>
          <Link href="#" className={`px-6 py-3 rounded-xl shadow font-bold transition
            ${darkMode ? 'bg-cyan-900 text-cyan-200 hover:bg-cyan-800' : 'bg-cyan-100 text-cyan-900 hover:bg-cyan-200'}`}
          >Riwayat</Link>
        </section>
        <style jsx global>{`
          @keyframes fadein {
